...
static const char *_STREAM_CHUNK_HEADER = "\r\n--" PART_BOUNDARY "\r\nContent-Type: image/jpeg\r\nContent-Length: %u\r\n\r\n";
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  enable_led(false);
#else
  Serial.println("[CAPTURE] Requesting esp_camera_fb_get()...");
  fb = esp_camera_fb_get();
  Serial.printf("[CAPTURE] Result: fb=%p (len=%u)\n", fb, fb ? fb->len : 0);
#endif
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  if (res != ESP_OK) {
    Serial.printf("[STREAM ERROR] set_type failed: %d\n", res);
    return res;
  }
  ...
  int frame_cnt = 0;
  while (true) {
    int64_t t_fb_start = esp_timer_get_time();
    fb = esp_camera_fb_get();
    if (!fb) {
      ...
      frame_cnt++;
      Serial.printf("[STREAM] Frame #%d: len=%u bytes\n", frame_cnt, fb->len);
    ...
    int64_t t_net_start = esp_timer_get_time();
    if (res == ESP_OK) {
      size_t hlen = snprintf(part_buf, sizeof(part_buf), _STREAM_CHUNK_HEADER, (uint32_t)_jpg_buf_len);
      res = httpd_resp_send_chunk(req, (const char *)part_buf, hlen);
    }
    if (res == ESP_OK) {
      res = httpd_resp_send_chunk(req, (const char *)_jpg_buf, _jpg_buf_len);
    }
    int64_t t_net_end = esp_timer_get_time();
    Serial.printf("[PERF #%d] fb_get: %lld ms | net_send: %lld ms | size: %u\n", 
                  frame_cnt, (t_net_start - t_fb_start)/1000, (t_net_end - t_net_start)/1000, _jpg_buf_len);
    if (fb) {
      esp_camera_fb_return(fb);
      fb = NULL;
      _jpg_buf = NULL;
...
void startCameraServer() {
  ...
  Serial.printf("[HTTP] Starting camera server on port: %u\n", config.server_port);
  esp_err_t res1 = httpd_start(&camera_httpd, &config);
  if (res1 == ESP_OK) {
    ...
    Serial.println("[HTTP] Camera server started successfully on port 80");
  } else {
    Serial.printf("[HTTP ERROR] Failed to start camera server: 0x%x\n", res1);
  }
  ...
  Serial.printf("[HTTP] Starting stream server on port: %u\n", config.server_port);
  esp_err_t res2 = httpd_start(&stream_httpd, &config);
  if (res2 == ESP_OK) {
    httpd_register_uri_handler(stream_httpd, &stream_uri);
    Serial.println("[HTTP] Stream server started successfully on port 81");
  } else {
    Serial.printf("[HTTP ERROR] Failed to start stream server: 0x%x\n", res2);
  }
}
